...
(function( $ ){
  ...
  var methods = {
    init : function( options ) {
	  	$this=$(this);
	  	var $newHeaderList=$($($.ajax({url: 'templates/service-list.xhtml', async: false}).responseText));
	  ...
	  //	//add triger to form in case user changes services
		console.log($("input[id='inputWSDL']").length);
	  	$("input[id='inputWSDL']").change(function(){alert('reload');$this.servicePlugin("reloadServices")});
...
function createServicesHTML(containers){
	...
	$.each(containers,function(index,value){
      ...
      if(label[1].substring(0, 1) == '_') {
		...
		textToInsert  += '<li class="serviceLi' + ((/*hide_aSync_serviceList && */isAsync) ? ' hideServiceItem' : '' ) + '" rel="' + value.label + '">'+label;
		//nested input
		textToInsert += "<ul>";
